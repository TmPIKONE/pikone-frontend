...
import { CalendarDays, Camera, ChevronDown, ImagePlus } from 'lucide-react';
...
import { useAnalyzeImage, useUpdateRecord } from '~/features/records/record.queries';
...
import type { RecordEditSheetProps } from './RecordEditSheet.types';
import * as S from './RecordEditSheet.styles';
...
const toRestaurantCandidate = (record: RecordEditSheetProps['record']): RestaurantCandidate => ({
  ...
  return (
    <>
      <BottomSheet
        isOpen={isOpen}
        title="기록 수정"
        ariaLabel="식사 기록 전체 수정"
        closeLabel="기록 수정 닫기"
        actionLabel={isSaving ? '저장 중...' : '수정 완료'}
        actionDisabled={!foodName.trim() || isPhotoUploading || isSaving}
        onClose={onClose}
        onAction={handleSave}
      >
        <S.Form>
